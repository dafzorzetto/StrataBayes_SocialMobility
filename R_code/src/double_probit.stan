...
parameters {
  ...
  vector[L_y] gamma3;                   // P0 - component means
  vector[L_y] gamma4;                   // P1*tr - component means
...
transformed parameters {
  ...
  for (n in 1:N) {
    ...
    vector[L_y-1] eta_y = beta0_y + beta1_y * X[n,]';
...
model {
  ...
  for (n in 1:N) {
    ...
    for (k_p in 1:L_p) {
      ...
      // likelihood
      lps_p[k_p] = log(weights_p[n, k_p]) + normal_lpdf(P[n] | alpha_linear + alpha2[k_p] * Tr[n], sigma_p[k_p]);
    ...
    P0_sim = sum(cond_mod_0_p); 
    P1_sim = sum(cond_mod_1_p); 
    
    // likelihood for Y
    for (k_y in 1:L_y) {
      real gamma_linear = gamma0[k_y] + dot_product(gamma1[k_y,], X[n,]) + gamma2[k_y] * Tr[n];
      
      gamma_linear += gamma3[k_y] * P0_sim + gamma4[k_y] * P1_sim * Tr[n]; 
...
generated quantities{
  ...
  for (n in 1:N) {
    ...
    // simulation P
    for (j in 1:L_y) {
      real gamma_linear_common = gamma0[j] + dot_product(gamma1[j,], X[n,]) + gamma3[j] * cond_mean_p0[n];
      real gamma_tr1 = gamma2[j] + gamma4[j] * cond_mean_p1[n];
      
      cond_mod_0_y[j] = weights_y[n, j] * (gamma_linear_common);
      cond_mod_1_y[j] = weights_y[n, j] * (gamma_linear_common + gamma_tr1);
    }
