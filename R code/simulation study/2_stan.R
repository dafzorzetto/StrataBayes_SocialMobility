...
# Stan code
setwd("../src")
probitDP_double <- stan_model("double_probit.stan")
...
L <- 5                      # DP truncation point fro both DDP
iter_Stan <- 2000           # total iterations for Stan models
...
estimation_Stan_PY <- function(data, model){
  ...
  init_function <- function() {
    ...
    list(
      ...
      beta1_p = matrix(rnorm((data_stan$L_p - 1) * data_stan$Q, 0, 0.3), 
                       data_stan$L_p - 1, data_stan$Q),
      
      beta0_y = rnorm(data_stan$L_y - 1, -0.5, 0.5),
      beta1_y = matrix(rnorm((data_stan$L_y - 1) * data_stan$Q, 0, 0.3), 
                       data_stan$L_y - 1, data_stan$Q),
      beta_p_y = rnorm(data_stan$L_y - 1, -0.5, 0.5),
      
      # Component parameters for P
      alpha0 = rnorm(data_stan$L_p, mean(data_stan$P), sd(data_stan$P) * 0.5),
      alpha1 = matrix(rnorm(data_stan$L_p * data_stan$Q, 0, 0.5), data_stan$L_p, data_stan$Q),
      alpha2 = rnorm(data_stan$L_p, 0, 0.5),
      alpha_ps = rnorm(data_stan$L_p, 0, 0.5),
      sigma_p = rgamma(data_stan$L_p, 2, 1),
      
      # Component parameters for Y
      gamma0 = rnorm(data_stan$L_y, mean(data_stan$Y), sd(data_stan$Y) * 0.5),
      gamma1 = matrix(rnorm(data_stan$L_y * data_stan$Q, 0, 0.5), data_stan$L_y, data_stan$Q),
      gamma2 = rnorm(data_stan$L_y, 0, 0.5),
      gamma3 = rnorm(data_stan$L_y, 0, 0.5),
      gamma4 = rnorm(data_stan$L_y, 0, 0.5),
      gamma_ps = rnorm(data_stan$L_y, 0, 0.5),
      sigma_y = rgamma(data_stan$L_y, 2, 1),
      
      # Hyperparameters for P
      mu0_a = mean(data_stan$P),
      mu1_a = rnorm(data_stan$Q, 0, 0.5),
      mu2_a = rnorm(1, 0, 0.5),
      mu_ps_a = rnorm(1, 0, 0.5),
      ...
      mu1_g = rnorm(data_stan$Q, 0, 0.5),
      mu2_g = rnorm(1, 0, 0.5),
      mu3_g = rnorm(1, 0, 0.5),
      mu4_g = rnorm(1, 0, 0.5),
      mu_ps_g = rnorm(1, 0, 0.5),
...
load("~/Documents/GitHub/StrataBayes/R code/temporal results/DEF_data_sim.RData")

est_s1 <- mclapply(scenario_1, function(d) 
  estimation_Stan_PY(data=d$data, model=probitDP_double), mc.cores = 6)
est_s2 <- mclapply(scenario_2_3_2_X, function(d)
  estimation_Stan_PY(data=d$data, model=probitDP_double), mc.cores = 6)
est_s3 <- mclapply(scenario_3, function(d) 
  estimation_Stan_PY(data=d$data, model=probitDP_double), mc.cores = 6)

save(est_s1, file = "results_doubleBNP_s1.RData")
save(est_s2, file = "results_doubleBNP_s2.RData")
save(est_s3, file = "results_doubleBNP_s3.RData")

PS_prova_s1 <- estimation_Stan_PY(data=scenario_1[[1]]$data, model=probitDP_double_PS_ps)
Med_prova_s1 <- estimation_Stan_PY(data=scenario_1[[1]]$data, model=probitDP_double_med_ps)
...
hist_comparison_1sample(data=scenario_1[[1]]$simulated_full, est=est_s1[[1]])
...
results_s1 <- lapply(1:200, function(x) est_resullts(data = scenario_1[[x]],
                                                    est=est_s1[[x]]) )
results_s2 <- lapply(1:200, function(x) est_resullts(data = scenario_2_3_2_X[[x]],
                                                    est=est_s2[[x]]) )
results_s3 <- lapply(1:200, function(x) est_resullts(data = scenario_3[[x]],
                                                    est=est_s3[[x]]) )
...
table_s1 <- table_results(results_s1)
table_s2 <- table_results(results_s2)
table_s3 <- table_results(results_s3)
