...
scenario_1_2_X <- lapply(1:samples, function(c) sim_bin_2_X(seed = c, n = n))
scenario_1_3_X <- lapply(1:samples, function(c) sim_bin_3_X(seed = c, n = n))



### --- scenarios based on second one ---
sim_bin_cont_2_X <- function(seed, n) {
  # set seed for reproducibility
  set.seed(seed)
  ...
  # Outcome
  #create for every unit a mean vector 
  mean_vector_0 <- rowSums(cbind(X[,-c(2:3)],P_0)*eta_0[V,])
  mean_vector_1 <- rowSums(cbind(X[,-c(2:3)],(P_1-P_0)^2)*eta_1[V,])
  Y_0 <- rnorm(n,mean_vector_0, sqrt(sigma_y_0[V,]))
  Y_1 <- rnorm(n,mean_vector_1, sqrt(sigma_y_1[V,]))
...
sim_bin_cont_3_X <- function(seed, n) {
  # set seed for reproducibility
  set.seed(seed)
  ...
  eta_1 <- matrix(c(10,1,1,0.1, 0.6,0.1, 0.2,-0.4,0.3,2.5,
  ...
  #create for every unit a mean vector 
  mean_vector_0 <- rowSums(cbind(X[,-c(2:3)],P_0)*eta_0[V,])
  mean_vector_1 <- rowSums(cbind(X[,-c(2:3)],(P_1-P_0))*eta_1[V,])
  Y_0 <- rnorm(n,mean_vector_0, sqrt(sigma_y_0[V,]))
  Y_1 <- rnorm(n,mean_vector_1, sqrt(sigma_y_1[V,]))
...
scenario_2_2_X <- lapply(1:samples, function(c) sim_bin_cont_2_X(seed = c, n = n))
scenario_2_3_X <- lapply(1:samples, function(c) sim_bin_cont_3_X(seed = c, n = n))
...
hist_simulation(scenario_1_3_X)
hist_simulation(scenario_1_2_X)

hist_simulation(scenario_2)
hist_simulation(scenario_2_3_X)
hist_simulation(scenario_2_2_X)
